...
def _auto_synthesize_begin_rendering(messages: list[dict]) -> list[dict]:
    """If the model emitted surfaceUpdate without beginRendering, infer and prepend beginRendering."""
    if any("beginRendering" in m for m in messages):
        return messages
                ...
                if isinstance(c, dict) and "id" in c:
                    ...
                    for spec in comp.values():
                        if isinstance(spec, dict):
                            if isinstance(spec.get("child"), str):
                                referenced_ids.add(spec["child"])
                            children = spec.get("children")
                            if isinstance(children, dict):
                                for cid in children.get("explicitList") or []:
                                    if isinstance(cid, str):
                                        referenced_ids.add(cid)
            root_candidates = list(defined_ids - referenced_ids)
            card_roots = [
                c["id"]
                for c in components
                if c.get("id") in root_candidates and "Card" in c.get("component", {})
            ]
            root_id = (
                card_roots[0]
                if card_roots
                else (root_candidates[0] if root_candidates else None)
            )
            if root_id:
                begin_msg = {
                    "beginRendering": {"root": root_id, "surfaceId": surface_id}
                }
                return [begin_msg] + messages
    return messages
...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        messages = _auto_synthesize_begin_rendering(messages)
